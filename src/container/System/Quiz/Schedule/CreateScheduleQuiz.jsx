...
export default function QuizScheduleForm() {
    ...
    try {
      ...
      if (res && res.success === true) {
        const formatted = res.data.map((cls) => ({
          label: `${cls.class_code}_${cls.class_name}`,
          value: +cls.id,
        }));
  ...
  const validateForm = () => {
    const newErrors = {};
    if (!form.quiz) newErrors.quiz = "Vui lòng nhập tên đề thi";
    if (!form.class_id) newErrors.class_id = "Vui lòng chọn lớp thi";
    if (!form.start_time)
  ...
  const handleSubmit = async () => {
    ...
    if (validateForm()) {
      try {
        ...
        if (res.success === true) {
          toast.success("Lưu lịch kiểm tra thành công");
          handleReset();
          setTimeout(() => {
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Tạo lịch thi mới</h1>
          <p className="text-sm text-gray-500">
            Điền thông tin chi tiết để tạo lịch thi cho thực tập sinh
          </p>
        </div>
      </div>
      ...
      <div className="space-y-8">
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 border border-gray-300 space-y-6">
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              {errors.quiz && (
                <p className="mt-1 text-xs text-red-600 flex items-center gap-1">
                  <AlertCircle className="w-3 h-3" /> {errors.quiz}
                </p>
              )}
            </div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
                Lớp thi <span className="text-red-500">*</span>
              </label>
              <select
                name="class_id"
                value={form.class_id}
